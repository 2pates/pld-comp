    ...
    switch (op) {
        ...
        case copy:
            ...
            if(target == Target::x86)
            {
                ...
                o << "mov" << size_to_letter(destination.size) << " %ebx," << to_string(destination.address) << "(%rbp)" << endl;
            }
            break;
        case add:
...
:cfg(cfg),entry_label(entry_label)
{}
...
void BasicBlock::gen_asm(ostream &o, Target target)
{
    if(target == Target::x86)
    {
        ...
        else if(exit_false != nullptr && exit_true != nullptr)
        {
            ...
            VariableInfo testVar = bb->cfg->get_var_info(test_var_name);
...
{
    IRInstr instruction(this, op, t, params);
    instrs.push_back(instruction);
}

CFG::CFG(string entryBlockLabel)
:entryBlockLabel(entryBlockLabel), nextBBnumber(0)
{}
...
void CFG::gen_asm_prologue(ostream& o, Target target)
{
    if(target == Target::x86)
    {
        ...
        o << "jmp " << entryBlockLabel << endl; // Jump to entry block
    }
}
...
void CFG::add_to_symbol_table(string name, VariableInfo t)
{
    variables[name] = t;
}
